<h1 align="center">Hi 👋, I'm Prateek Sharma</h1>
<h3 align="center">Full Stack Developer | React • Django • Python • JavaScript</h3>

<p align="center">
📍 Ghaziabad, Uttar Pradesh, India  
</p>

<p align="center">
  <a href="mailto:prateekps0123@gmail.com">
    <img src="https://img.shields.io/badge/Email-prateekps0123%40gmail.com-red?style=for-the-badge&logo=gmail">
  </a>
  <a href="https://www.linkedin.com/in/prateek-sharma-45851a201/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Prateek%20Sharma-blue?style=for-the-badge&logo=linkedin">
  </a>
  <a href="https://github.com/MrSharma144" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-PrateekSharma-black?style=for-the-badge&logo=github">
  </a>
  <a href="https://leetcode.com/u/MrSharma144/" target="_blank">
    <img src="https://img.shields.io/badge/LeetCode-150%2B%20Problems-orange?style=for-the-badge&logo=leetcode">
  </a>
</p>

---

## 🚀 About Me

I'm a **passionate Full Stack Developer** who loves building **scalable, user-friendly web applications** using modern technologies.

- 🔥 Strong focus on **clean code & performance**
- ⚙️ Experienced with **frontend + backend integration**
- 🌱 Always learning and exploring new tech
- 💡 Enjoy solving real-world problems through code

---

## 🛠️ Tech Stack

### 💻 Languages
![Python](https://img.shields.io/badge/Python-blue?style=flat&logo=python)
![Java](https://img.shields.io/badge/Java-red?style=flat&logo=java)
![JavaScript](https://img.shields.io/badge/JavaScript-yellow?style=flat&logo=javascript)
![HTML](https://img.shields.io/badge/HTML-orange?style=flat&logo=html5)
![CSS](https://img.shields.io/badge/CSS-blue?style=flat&logo=css3)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-blue?style=flat&logo=postgresql)

### ⚙️ Frameworks & Tools
![React](https://img.shields.io/badge/React-black?style=flat&logo=react)
![Tailwind](https://img.shields.io/badge/TailwindCSS-blue?style=flat&logo=tailwindcss)
![Django](https://img.shields.io/badge/Django-green?style=flat&logo=django)
![Git](https://img.shields.io/badge/Git-orange?style=flat&logo=git)

### 🧰 Developer Tools
- VS Code  
- GitHub  
- Postman  
- Render  
- Vercel  
- Jupyter Notebook  

---

## 🚀 Projects

### 🔹 **WorkShala – Job & Internship Portal**
🔗 *Project Link: https://workshala-in.vercel.app/*  
**Tech:** React, JavaScript, Python, Django REST APIs  

- Built a full-featured job & internship platform (Internshala-like)
- Implemented user authentication & authorization
- Resume upload, job search & filtering
- Application tracking system
- Fully responsive React frontend with REST API integration

---

### 🔹 **IntelliBot – AI Powered Chatbot**
🔗 *Project Link: https://github.com/MrSharma144/React-AI-Tool*  
**Tech:** React, JavaScript, Tailwind CSS, Gemini API, Regex  

- Developed an AI chatbot with smooth UI & UX
- Integrated Google Gemini API for real-time responses
- Regex-based response formatting for better readability
- Clean and modern chat interface

---

## 💼 Experience

### 🏢 **BlueStock FinTech** — Frontend Developer Intern *(Remote)*  
📅 **Sep 2025 – Nov 2025**

- Built a responsive NGO website using React & Tailwind CSS
- Added animations, hover effects & dynamic UI components
- Followed Git/GitHub workflows and agile practices
- Focused on modern UI/UX design principles

---

### 🧠 **IBM SkillsBuild** — Project Based Learning  
📅 **July 2025 – August 2025**

- Built full-stack applications using React & JavaScript
- Designed PostgreSQL schemas & CRUD operations
- Optimized SQL queries for performance

---

## 🎓 Education

🎓 **B.Tech – Computer Science & Engineering (Data Science)**  
🏫 Ajay Kumar Garg Engineering College *(2022 – 2026)*  
📊 **CGPA: 8.31**

---

## 🏆 Achievements

- 🧠 Solved **150+ LeetCode problems**
- 🚀 Self-learned React & Node.js with real projects
- 🥇 **Winner – Inter-School Kho-Kho Competition (2020)**
- 🤝 Strong communication & team collaboration skills

---

## 📫 Let’s Connect

- 📧 **Email:** [prateekps0123@gmail.com](mailto:prateekps0123@gmail.com)
- 💼 **LinkedIn:** https://www.linkedin.com/in/prateek-sharma-45851a201/  
- 🧑‍💻 **GitHub:** https://github.com/MrSharma144  
- 🧠 **LeetCode:** https://leetcode.com/u/MrSharma144/  

---

⭐ **If you like my work, don’t forget to star my repositories!**
